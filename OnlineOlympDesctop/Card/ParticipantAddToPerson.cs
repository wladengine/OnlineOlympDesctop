...
{
    public partial class ParticipantAddToPerson : Form
    {
        Guid PersonId;
        UpdateHandler hdl;
        public ParticipantAddToPerson(Guid Id, UpdateHandler h)
        {
            InitializeComponent();
            PersonId = Id;
            hdl = h;
            ...
            using (OnlineOlymp2016Entities context = new OnlineOlymp2016Entities())
            {
                ...
                var lst = (from x in context.Participant
                           ...
                           select new
                           {
                               ...
                               (x.Id.ToString(), 
                               x.Name + " (" + x.Class+ ")," + x.Nationality + ", " + x.Region)).ToList();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            using (OnlineOlymp2016Entities context = new OnlineOlymp2016Entities())
            {
                var Part = (from x in context.Participant
                            where x.Id == PartId
                            select x).FirstOrDefault();
                if (Part == null)
                    return;
