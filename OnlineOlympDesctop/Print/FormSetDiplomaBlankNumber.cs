...
{
    public partial class FormSetDiplomaBlankNumber : Form
    {
        public event Action OnOK;
        private Guid OlympDiplomaId;
        public FormSetDiplomaBlankNumber(Guid gOlympDiplomaId)
        {
            InitializeComponent();
            ...
            using (OlympVseross2016Entities context = new OlympVseross2016Entities())
            {
                tbValue.Text = context.OlympDiploma.Where(x => x.Id == gOlympDiplomaId)
                    .Select(x => x.BlankNumber).DefaultIfEmpty("").First();
            }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            using (OlympVseross2016Entities context = new OlympVseross2016Entities())
            {
                ...
                if (val != null)
                    val.BlankNumber = tbValue.Text.Trim();
